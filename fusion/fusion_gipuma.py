...
parser.add_argument('--list_file', type=str,
                    ...
                    help='file contains the scans list')
...
parser.add_argument('--quan_path', type=str, default='')
...
parser.add_argument('--matlab_quantitative', action='store_true')
parser.add_argument('--only_quantitative', action='store_true')
parser.add_argument('--quan_root_dir', type=str, default='/media/public/yan1/doublez/realdoubleZ/Data/MVS/evaluation/')
...
def collectPly(point_folder, scan_id):
    ...
    fresh = os.path.join(args.ply_path, "mvsnet") + scan_id.zfill(3) + ".ply"
    shutil.move(old, fresh)


def matlabQuantitative(ply_id):
    os.system("pwd")
    command = '''matlab -nodesktop -nosplash -r "cd evaluation/dtu_parallel; dataPath='{}'; plyPath='{}'; resultsPath='{}'; method_string='{}'; thisset='{}'; BaseEvalMain_web" &'''.format(
        args.quan_root_dir,
        os.path.join("../../", args.ply_path), 
        os.path.join("../../", args.quan_path),
        "mvsnet", ply_id
    )
    os.system(command)
...
if __name__ == '__main__':
    ...
    if not os.path.isdir(args.quan_path):
        os.mkdir(args.quan_path)
    ...
    for i, scan in enumerate(scans):
        ...
        if  args.matlab_quantitative:
            print("Start matlab quantitative for {}".format(scan))
            matlabQuantitative(scan[4:])

...
class Affinity_Propagate(nn.Module):
    ...
    def forward(self, guidance, blur_depth, sparse_depth=None):
        ...
        self.sum_conv = nn.Conv3d(in_channels=8,
                                  ...
                                  padding=0,
                                  bias=False)
        weight = torch.ones(1, 8, 1, 1, 1).to(blur_depth.device)#.cuda()
...
if __name__ == "__main__":
    net = Affinity_Propagate(24, 3, "8sum").cuda()

    batch = 2
    # w, h = 160, 128
    w, h = 1600, 1024
    cvp_depth = torch.randn(batch, 1, w, h).cuda()
    guidance = torch.randn(batch, 8, w, h).cuda()   # 8 channels feature map
    sparse_depth = None # torch.randn(batch, 1, w, h).cuda()    Sparse depth map, can be omitted

    final_depth = net(guidance, cvp_depth, sparse_depth)
